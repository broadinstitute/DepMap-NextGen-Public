...
def load_data():
    """
    Load all data necessary to make celligner-related plots
    """
...
def force_categories(series, category_list):
    """
    Replace any non-specified labels with "Other"

    Args:
        series (pandas.Series): A series of labels
        category_list (str[list]): A list of valid labels; any labels in `series` that are not in this list are recategorized

    Returns:
        pandas.Series: A series of recategorized labels
    """
...
def global_celligner(lineages, all_celligner_samples, omics_models_meta, screen_metadata):
    ...
    Args:
        lineages (list[str]): A list of lineages to highlight 
        all_celligner_samples (pandas.DataFrame): A table of celligner coordinates per sample 
...
def pdac_celligner(all_celligner_samples, omics_models_meta, all_mmp_scores):
    ...
    (from Gavish et al., 2023).

    Args:
        all_celligner_samples (pandas.DataFrame): A table with celligner coordinates per sample
        omics_models_meta (pandas.DataFrame): A table of model metadata
        all_mmp_scores (pandas.DataFrame): A matrix (sample x geneset) of metaprogram expression scores
    """
...
def celligner_classification(next_gen_lineage_order, celligner_top_nns):
    """
    Plot a square confusion matrix of lineage classification results from kNN classification on celligner coordinates

    Args:
        next_gen_lineage_order (list[str]): A list of lineages defining their order of appearance in the confusion matrix
        celligner_top_nns (pandas.DataFrame): A table of kNN classification results
    """
    ...
    def generate_normalized_confusion_matrix(input_boolean_column, max_rank):
        """
        Generates a confusion matrix from kNN classification results

        Args:
            input_boolean_column (str): Label column to count over
            max_rank (int): Maximum number of neighbors to consider

        Returns:
            pandas.DataFrame: A matrix (predicted lineage x true lineage) of class prediction counts
        """
        # filter input models
...
def celligner_extended_classification(next_gen_lineage_order, celligner_top_nns):
    ...
    from kNN classification on celligner coordinates
...
def main():
    """
    Generate all plots related to celligner
    """
