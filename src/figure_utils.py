...
def approximate_fdr_in_p_units(df, fdr_threshold=0.05, pcolumn='pvalue', fdr_column='FDR'):
    """
    Given a set of q-values, approximates the corresponding p-value threshold using the median of the nearest p-values to the significane boundary
    """
...
def search_annotations_index(texts, query_list):
    """
    Locates the indices of the text objects corresponding to the requested labels
    """
    indices = []
...
def manually_annotate(ax, text, df, xcol, ycol, xyoffset, xycoords='data', 
                      ...
                      bbox_kws=None):
    """
    Manual control over text positions, parameterized by an anchor point and an offeset relative to the anchor point; defaults to using data coordinates
    """
    ax.annotate(
...
def horizontal_annotate_significance_bar(ax, string, x, y_pair, text_offset=0.5, cap_length=0.5, linewidth=0.5):
    """
    Produces a figure element resembling a signficance bar
    """
...
def make_gene_effect_distribution_plot(genes, mtx=None,
    ...
    """
    Produces a vertical plot showing the 1D distributions of input values; defaults assume the purpose of plotting gene effects per gene or per category
    """
