...
def search_gene(query_gene, gene_list=hgnc_annotations['cds_gene_id'].tolist(), prefix=None):
    """
    Finds the most likely match to a gene name in a list of gene tags. Attempts exact search before resorting to partial matching.
    """
...
def connected_component(adjacency_matrix, source):
    """
    Finds all elements in the same connected component as the source element, given an adjacency matrix.
    """
...
def partition_by_connected_component(adjacency_matrix):
    """
    Given an adjacency matrix, finds all connected components
    """
    if type(adjacency_matrix) == pd.DataFrame:
...
def clean_geneset_name(string, nth=2, remove_prefix=True):
    """
    Parses a gene set name to replace it with a cleaner version, removing the prefixed collection, replacing underscores with spaces, and adding newlines
    """
