...
def plot_library_correction_examples(screen_metadata, ngs, screen_gene_effect):
    ...
    minlim = min(ngs_hu_vs_av.min().min(), screen_hu_vs_av.min().min(), aligned_hu_vs_av.min().min())
    maxlim = min(ngs_hu_vs_av.max().max(), screen_hu_vs_av.max().max(), aligned_hu_vs_av.max().max())
