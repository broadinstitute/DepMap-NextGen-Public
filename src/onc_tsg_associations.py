...
def plot_onc_tsg_volcano(fdr_threshold=0.05, mean_difference_threshold=0.3):
    ...
    Args:
        fdr_threshold (float, optional): Minimum significance threshold. Defaults to 0.05.
        mean_difference_threshold (float, optional): Minimum effect size threshold. Defaults to 0.3.
    """
...
def create_paired_violin_plot(label, onc_gof_altered_mtx, tsg_lof_altered_mtx, screen_gene_effect, screen_metadata, next_gen_screen_list, adherent_screen_list, next_gen_onc_tsg_test_df, ax_map_panel):
    """
    Plot a paired violin plot comparing dependencies across mutation status of a biomarker in NextGen models and Traditional adherent models

    Args:
        label (str): A label of "<dependency>/<biomarker>"
        onc_gof_altered_mtx (pandas.DataFrame): A binary matrix (sample x gene) indicating the presence of a gain-of-function mutation
        tsg_lof_altered_mtx (pandas.DataFrame): A binary matrix (sample x gene) indicating the presence of a loss-of-function mutation
        screen_gene_effect (pandas.DataFrame): A matrix (screen x gene) of gene effects
        screen_metadata (pandas.DataFrame): A table of screen metadata
        next_gen_screen_list (list[str]): A list of NextGen screens
        adherent_screen_list (list[str]): A list of Traditional adherent screens
        next_gen_onc_tsg_test_df (pandas.DataFrame): A table of two-class comparison test results between mutant and WT screens for different genes
        ax_map_panel (dict[(int, int) -> matplotlib.pyplot.Axes]): A mapping of figure axes

    Returns:
        pandas.DataFrame: A table with values underlying the violin plots
    """
    ...
    # reprioritize lineage z-stack order dependending on the biomarker
    if "APC" in biomarker:
        next_gen_order = (
            screen_metadata[
    ...
    # join all values into an output table
...
def plot_example_panel(examples, 
    ...
    """
    Plot example violin plots for the selected pairs of dependencies and mutation biomarkers

    Args:
        examples (list[str]): A list of labels formatted as "<dependency>/<biomarker>"
        figsize (tuple, optional): Figure dimensions to create. Defaults to (80 * mm, 50 * mm).
        column_ratios (list, optional): Relative horizontal sizes of the paired plots. Defaults to [1, 1].
        column_widthspace (float, optional): Horizontal width between the subplots. Defaults to 0.5.
    """
...
def plot_onc_tsg_alteration_example(dep, feat):
    """
    Plot a scatterplot of a dependency-biomarker relationship that depends on either mutation status or copy number alteration

    Args:
        dep (str): Dependency label
        feat (str): Biomarker label
    """
    screen_metadata = load_file('screen_metadata.csv', index_col=0)
    ...
    # create tables for plotting example relationships
    def construct_example_df(dependency, biomarker, screen_metadata_subset):
        ...
        Returns:
            pandas.DataFrame: A table of paired dependency-copy-number values per sample
        """
    ...
    # define color groupings and apply log transformation to copy number
