...
def tda_classify_common_essentials(gene_effect_matrix, quantile=0.9):
    """
    Given a matrix (cell lines x genes) of gene effect scores, identifies a list of common essential genes, defined as having a depletion score 
    in the top X genes per cell line for at least Q% of cell lines, where X is chosen based on the distribution of Qth percentile gene ranks

    Args:
        gene_effect_matrix (pandas.DataFrame): A matrix (cell line x gene) of gene effect scores
        quantile (float, optional): Quantile (Q) at which to evaluate gene depletion effects. Defaults to 0.9.

    Returns:
        pandas.DataFrame: A table with the Qth percentile least-depleting rank for a gene across all cell line s
    """
    ...
    percentile = rank_data.quantile(q=quantile) # Compute the Qth percentile for each column in the DataFrame
    
    # Calculate the density function of the Qth percentile values
    kde = scipy.stats.gaussian_kde(percentile.dropna(), bw_method=0.3)
    test_points = np.arange(0.1, 0.9, 0.001)
    threshold = test_points[np.argmin(kde(test_points))] # select the minimum of the bimodal distribution of Qth percentile gene ranks
...
def tda_classify_strongly_selective(gene_effect_matrix, probability_dependency_matrix, probability_threshold=0.5, selectivity_threshold=-0.86):
    """
    Given matrices (cell lines x genes) of gene effect scores and probability of dependency, identifies a list of strongly selective
    genes, defined as having a skewness and kurtosis product below a given threshold, indicating a long negative tail of gene effect 
    scores, and exhibiting dependence in at least one cell line

    Args:
        gene_effect_matrix (pandas.DataFrame): A matrix (cell line x gene) of gene effect scores
        probability_dependency_matrix (pandas.DataFrame): A matrix (cell line x gene) of dependency probabilities
        probability_threshold (float, optional): The minimum probability to be a dependency, defined on [0, 1]. Defaults to 0.5.
        selectivity_threshold (float, optional): The maximum product of skew and kurtosis to be a strongly selective gene. Defaults to -0.86.

    Returns:
        pandas.DataFrame: A table of labels for strong selectivity, in addition to summary statistics across cell lines used in the classification
    """
    ...
    # statistical measures on the gene effect matrix
    ...
    # statistical measures on the probability matrix
    ...
    # classification of strong selectivity
...
def tda_classify_high_variance(gene_dependency_matrix, expr_matrix):
    """
    Given matrices (cell lines x genes) of probability of dependency and gene expression, identifies a list of high-variance genes, 
    defined as having higher variance in dependency across cell lines than a unexpressed (control) set of genes

    Args:
        gene_dependency_matrix (pandas.DataFrame): A matrix (cell line x gene) of dependency probabilities
        expr_matrix (pandas.DataFrame): A matrix (cell line x gene) of gene expression values

    Returns:
        pandas.DataFrame: A table of labels for high variance in dependency across cell lines
    """
    ...
    expr_cutoff = test_points[np.argmin(kde(test_points))] # select the minimum of the bimodal distribution of expression values of any gene in any cell line
...
def run_tda_classification(gene_effect_matrix, gene_dependency_matrix, expr_matrix, model_subsets = dict(), 
    ...
    """
    Classify genes with the non-exclusive labels of "common essential", "strongly selective", and "high variance", using only the sets of
    models defined by `model_subsets`

    Args:
        gene_effect_matrix (pandas.DataFrame): A matrix (cell line x gene) of gene effect scores
        gene_dependency_matrix (pandas.DataFrame): A matrix (cell line x gene) of dependency probabilities
        expr_matrix (pandas.DataFrame): A matrix (cell line x gene) of gene expression values
        model_subsets (dict[str -> list[str]], optional): A dictionary mapping group names to lists of cell line names. Defaults to dict().
        probability_threshold (float, optional): Threshold for minimum probability of dependency. Defaults to 0.5.
        selectivity_threshold (float, optional): Threshold for maximum skew * kurtosis. Defaults to -0.86.
        essentiality_quantile (float, optional): Threshold for evaluating essentiality across cell lines. Defaults to 0.9.

    Returns:
        pandas.DataFrame: A table of genes with classification labels and related statistics for each label
    """
    results_dict = dict()
...
def reclassify_tda_selective(results_df, min_frac_dep=None, min_n_dep=1, threshold=-0.86):
    """
    Reclassify strongly selective genes based on dependency requirements across cell lines.

    Args:
        results_df (pandas.DataFrame): An output from tda_classify_high_variance() or run_tda_classification()
        min_frac_dep (float, optional): The minimum fraction of cell lines showing dependency to classify as strongly selective. Defaults to None.
        min_n_dep (int, optional): The minimum number of cell lines showing dependency to classify as strongly selective. Defaults to 1.
        threshold (float, optional): The maximum skew * kurtosis to classify as strongly selective. Defaults to -0.86.

    Returns:
        pandas.DataFrame: A copy of the input table, with the `selective` column added based on the reclassification scheme.
    """
...
def partition_genes_by_tda_results(tda_results_df, selectivity_kws={}, paness_threshold=None,
    ...
    """
    Create strictly exclusive gene classifications based on summaries from earlier classification functions

    Args:
        tda_results_df (pd.DataFrame): An output from run_tda_classification()
        selectivity_kws (dict, optional): Key-value pairs for strongly selective gene classification (see reclassify_tda_selective). Defaults to {}.
        paness_threshold (float, optional): Quantile threshold on [0, 1] for recategorizing common essentials. Defaults to None.
        order (list, optional): Prioritization order for gene classification. Later labels will overwrite earlier labels. Defaults to ['Non-dependency', 'Weakly Selective', 'Pan-dependency', 'High Variance', 'Strongly Selective'].

    Returns:
        pandas.DataFrame: A table of gene classes and key criteria for each label
    """
    ...
    # define non-exclusive class labels
    ...
    # successively overwrite class labels for each class
...
def call_gene_classes():
    """
    Run procedure for classifying genes by dependency scores
    """
    # load data
...
def z_score_by_expression_bins(expression_matrix, nbin=25, n_sample=100, seed=None):
    """
    Calculates normalized expression values by z-scoring within expression bins against a random subset of control genes per bin

    Args:
        expression_matrix (pandas.DataFrame): A matrix (sample x gene) of expression values
        nbin (int, optional): The number of expression bins to create. Defaults to 25.
        n_sample (int, optional): The number of control genes per bin. Defaults to 100.
        seed (int, optional): A seed for the random number generator. Defaults to None.

    Returns:
        pandas.DataFrame: The input matrix, with entires normalized against genes with comparable expression
        pandas.DataFrame: A mapping of genes to their expression bins and a designation of control gene set membership
    """
    ...
    control_genes_by_bin = genes_to_bins.reset_index().rename({'index': 'gene', 0: 'bin'}, axis=1).groupby(
        'bin'
    ).apply(lambda x: x.sample(n=n_sample) if len(x) >= n_sample else x.sample(frac=1)).set_index('gene')['bin']
...
def compare_metaprograms():
    """
    Run two-class comparison hypothesis tests on geneset expression scores between NextGen models and Traditional 2D models of the corresponding lineage of origin
    """
    omics_models_meta = load_file('model_metadata.csv', index_col=0)
...
def identify_nearest_neighbors(dist_matrix, input_table, target_table, nn=25):
    """
    Use a distance matrix to identify the top nearest neighbors in the target set for each element of the input set

    Args:
        dist_matrix (pandas.DataFrame): A matrix of distances between elements
        input_table (pandas.DataFrame): A table whose indices are elements of the set of anchor points for which to identify nearest neighbors
        target_table (pandas.DataFrame): A table whose indices are potential neighbors of the input anchor points
        nn (int, optional): The number of top nearest neighbors to identify per input point. Defaults to 25.

    Returns:
        pandas.DataFrame: A table mapping input elements to nearest neighbor target elements
    """
...
def classify_by_nearest_neighbors(nearest_neighbor_table, target_column='lineage', 
    ...
    """
    Classify elements of the input class based using majority vote for an attribute of the neighbor class

    Args:
        nearest_neighbor_table (pandas.DataFrame): The output of identify_nearest_neighbors(), mapping input elements to their nearest neighbors
        target_column (str, optional): The attribute to use for label transfer. Defaults to 'lineage'.
        top_n (int, optional): The number of nearest neighbors to consider for classification. Defaults to 25.
        exclude_insufficient (bool, optional): Flag for whether to exclude inputs from classification if their nearest neighbors are majority unlabeled. Defaults to True.
...
def run_celligner_classification():
    ...
    # reindex the celligner distance matrix
    ...
    # reindex the corresponding metadata table
    all_depmap_models_celligner_table.set_index("ModelID", inplace = True)

    # perform lineage prediction on DepMap models using kNN
...
def compute_expression_addictions(screen_metadata_subset, screen_gene_effect, screen_expr):
    """
    Calculate correlations between gene effects scores and corresponding expression values for the same genes

    Args:
        screen_metadata_subset (pandas.DataFrame): A table indexed by screen IDs
        screen_gene_effect (pandas.DataFrame): A matrix (screen x gene) of gene effect scores
        screen_expr (pandas.DataFrame): A matrix (screen x gene) of gene expression values

    Returns:
        pandas.DataFrame: A table of correlations between gene effects and expression values per gene
    """
    # align matrix indices and columns
    ...
    # calculate correlations of all gene effect-expression pairs
...
def run_expression_addiction_analysis():
    """
    Perform biomarker analysis for the class of expression addiction dependencies. Calculates correlations between
    genes' fitness effect scores and their expression values.
    """
    screen_gene_effect = load_file('screen_gene_effect.csv', index_col=0)
    screen_metadata = load_file('screen_metadata.csv', index_col=0)
    expr = load_full_matrix('expression')
    ...
    # combine results between subsets
...
def run_paralog_dependency_analysis(paralog_table, gene_effect_matrix, biomarker_matrix, biomarker_type):
    """
    Calculate correlations of gene effect scores with gene expression or copy number of their paralogs

    Args:
        paralog_table (pandas.DataFrame): A table of paralog pairs to test
        gene_effect_matrix (pandas.DataFrame): A matrix (screen/cell line x gene) of gene effect scores
        biomarker_matrix (pandas.DataFrame): A matrix (screen/cell line x gene) of values to correlate gene effects against, typically gene expression
        biomarker_type (str): A string label describing the type of biomarker

    Returns:
        pandas.DataFrame: A table of correlations between input pairs' gene effects and expression
    """
...
def compute_paralog_dependencies(screen_metadata_subset, ensembl_paralogs, screen_gene_effect, screen_expr):
    """
    Run paralog analysis on a subset of screens

    Args:
        screen_metadata_subset (pandas.DataFrame): A table indexed by screen IDs
        ensembl_paralogs (pandas.DataFrame): A table of paralog pairs to test
        screen_gene_effect (pandas.DataFrame): A matrix (screen x gene) of gene effect scores
        screen_expr (pandas.DataFrame): A matrix (screen x gene) of gene expression values

    Returns:
        pandas.DataFrame: A table of correlations between input pairs' gene effects and expression over the screen subset
    """
...
def run_paralog_analysis():
    """
    Perform biomarker analysis for the class of paralog dependencies. Calculates correlations between genes' fitness effect 
    scores and the expression of their paralogs.
    """
    screen_gene_effect = load_file('screen_gene_effect.csv', index_col=0)
    screen_metadata = load_file('screen_metadata.csv', index_col=0)
    expr = load_full_matrix('expression')
    ...
    # combine results for each screen subset
...
def prepare_alteration_data(amplification_min_cn = 3, deletion_max_cn = 0.25):
    """
    Convert continuous-valued copy number data and boolean-valued mutation data into boolean matrices defining
    the presence of gene amplifications, deletions, hotspot mutations, and damaging mutations

    Args:
        amplification_min_cn (int, optional): Minimum relative copy number to classify as amplification. Defaults to 3.
        deletion_max_cn (float, optional): Maximum relative copy number to classify as deletion. Defaults to 0.25.

    Returns:
        tuple(pandas.DataFrame): A set of tables and matrices corresponding to each type of alteration.
    """
    ...
    adherent_amplified_mtx = adherent_cn_matrix > amplification_min_cn
    adherent_deleted_mtx = adherent_cn_matrix < deletion_max_cn
...
def run_oncogene_tsg_dependency_analysis(genes_to_test, screen_effect_matrix, alteration_matrix, dependency_matrix=None):
    """
    Perform two-class comparison hypothesis tests on gene effects relative to the presence of mutations in frequently
    altered oncogenes and tumor suppressors.

    Args:
        genes_to_test (list[str]): A list of genes to test
        screen_effect_matrix (pandas.DataFrame): A matrix (screen x gene) of gene effect scores
        alteration_matrix (pandas.DataFrame): A boolean matrix (screen x gene) of mutation presence
        dependency_matrix (pandas.DataFrame, optional): A matrix (screen x gene) of probability of dependency. Defaults to None.

    Returns:
        pandas.DataFrame: A table of two-class test results (Mann-Whitney U/Wilcoxon rank-sum and T-test) for genes with any 
            dependency against oncogene or tumor suppressor alterations
    """
    association_tests = []
...
def run_dependency_vs_lineage_biomarker_analysis(screen_effect_matrix, categorical_annotation, dependency_matrix=None):
    """
    Perform two-class comparison hypothesis tests on gene effects relative to categorical labels, using an
    all-vs-rest strategy for defining comparison groups.

    Args:
        screen_effect_matrix (pandas.DataFrame): A matrix (screen x gene) of gene effects.
        categorical_annotation (pandas.Series): A series of categorical labels defining the groupings, typically lineage of origin
        dependency_matrix (pandas.DataFrame, optional): A matrix (screen x gene) of probability of dependency. Defaults to None.

    Returns:
        pandas.DataFrame: A table of two-class hypothesis test results.
    """
...
def run_oncogene_tsg_association_analysis():
    """
    Perform biomarker analysis for the class of oncogene and tumor-suppressor dependencies. Calculates two-class
    comparison tests between gene expression scores in mutant/altered models against wild-type models for key oncogenes
    and tumor suppressors. Additionally tests lineage relationships for reference.
    """
    (
        cancer_gene_annotations, screen_gene_effect, screen_metadata,
        next_gen_screen_effects, nextgen_og_altered_mtx, nextgen_ts_altered_mtx,
...
def compute_gbm_cdk6_dependency_biomarker_tests(correlation_threshold=0.3, correlation_significance_threshold=0.005,
    ...
    """
    Performs biomarker analysis for CDK6 dependency in glioblastoma models

    Args:
        correlation_threshold (float, optional): Minimum effect size for correlations. Defaults to 0.3.
        correlation_significance_threshold (float, optional): Minimum significance value for correlations. Defaults to 0.005.
        min_mutations (int, optional): Minimum number of alterations to perform two-class comparison tests. Defaults to 5.
        mean_effect_threshold (float, optional): Minimum effect size difference between mutant and WT models. Defaults to 0.3.
        mwu_significance_threshold (float, optional): Minimum significance between mutant and WT models. Defaults to 0.05.
    """
    screen_metadata = load_file('screen_metadata.csv', index_col=0)
    screen_gene_effect = load_file('screen_gene_effect.csv', index_col=0)
    expr = load_full_matrix('expression')
    ...
    gbm_cdk6_dep_mut_tests['Significant'] = (
        (gbm_cdk6_dep_mut_tests['Mean Difference'].abs() > mean_effect_threshold) &
        (gbm_cdk6_dep_mut_tests['FDR'] < mwu_significance_threshold)
    )
...
def run_pdac_classical_analysis():
    """
    Calculates gene effects correlated with the PDAC-classical program (from Gavish, 2023)
    """
    ...
    # index the matrices appropriately for comparison
...
def compare_next_gen_vs_adherent_dependency():
    """
    Perform two-class comparisons between NextGen models and traditional adherent models
    """
    screen_gene_effect = load_file('screen_gene_effect.csv', index_col=0)
    screen_gene_dependency = load_file('screen_gene_dependency.csv', index_col=0)
    screen_metadata = load_file('screen_metadata.csv', index_col=0)
...
def generate_sequence_map(sequence_metadata, pdna_sequence_ids=['HEK_CasNull_MP_control']):
    """
    Generate the table defining samples and their conditions, required as input to Chronos

    Args:
        sequence_metadata (pandas.DataFrame): A table of metadata for minipool screens
        pdna_sequence_ids (list[str], optional): The labels used to denote pDNA samples. Defaults to ['HEK_CasNull_MP_control'].

    Returns:
        pandas.DataFrame: A sequence metadata table, formatted for compatibility with Chronos
    """
...
def compute_nnmd(df, poscons, negcons):
    """
    Calculates the NNMD (null-normalized median difference) metric:
        median(poscons) - median(negcons) / MAD(negcons)
    """
...
def compute_roc_auc(x, poscons, negcons):
    """
    Calculates the ROC-AUC (area under the receiver-operator curve) metric, measuring the separability of 
    positive control and negative control values
    """
...
def normalize_chronos_gene_effects(gene_effect_matrix, 
                                   ...
                                   individual=False):
    """
    Normalizes a raw gene effect matrix by centering negative control values at 0 and positive control genes at -1.

    Args:
        gene_effect_matrix (pandas.DataFrame): A matrix (screen x gene) of gene effect scores, output by chronos
        negative_control_genes (list[str]): A list of negative control genes, expected to not have any fitness effect
        positive_control_genes (list[str]): A list of positive control genes, expected to have sstrong fitness effects in any cell line
        individual (bool, optional): Whether to perform the normalization per screen (True) or per library (False). Defaults to False.

    Returns:
        pandas.DataFrame: A normalized matrix of gene effect scores.
    """
...
def gene_effect_linear_regression(gene_dependency_matrix, metadata_table, exogenous_variables, 
    ...
    """
    Fits a linear regression model on gene effect (or dependency) scores against a table of covariates for each gene. 
    Extracts coefficients for the relationships between gene effects and covariates.

    Args:
        gene_dependency_matrix (pandas.DataFrame): A matrix (screen x gene) of gene effect scores
        metadata_table (pandas.DataFrame): A table of covariates, indexed like `gene_dependency_matrix`
        exogenous_variables (list[str]): A list of column names for covariates to use as independent features
        combination_variables (list[list[str]]): A list of column name sets for higher-order terms in the regression model. Defaults to [].

    Returns:
        pandas.DataFrame: A table of coefficients in the linear regression model for each gene
    """
...
def f_test_variance(x, y, alternative='two-sided'):
    """
    Performs an F-test for the equality of variance between two groups of values

    Args:
        x (Iterable[float]): Values in the first group
        y (Iterable[float]): Values in the second group
        alternative (str, optional): The direction of the alternative hypothesis. Defaults to 'two-sided'.

    Raises:
        ValueError: Raises an error if the direction of the alternative hypothesis is unrecognized
...
def run_minipool_qc(readcounts, sample_metadata, guide_metadata, test_guides, neg_ctrl_guides, pos_ctrl_guides):
    """
    Performs quality control on readcounts for CRISPR screens

    Args:
        readcounts (pandas.DataFrame): A matrix (sample x guide) of readcounts
        sample_metadata (pandas.DataFrame): A table of sample metadata, indexed like `readcounts`
        guide_metadata (pandas.DataFrame): A table of guide metadata, indexed with the columns of `readcounts`
        test_guides (list[str]): A list of non-control guides
        neg_ctrl_guides (list[str]): A list of negative control guides, expected to not have fitness effects
        pos_ctrl_guides (list[str]): A list of positive control guides, expected to have strong fitness effects

    Returns:
        pandas.DataFrame: A table of sample-level quality control metrics
        pandas.DataFrame: A table of replicate-level quality control metrics
    """
    ...
    # calculate LFC at the screen level
    ...
    # define replicate-to-sample mappings
    ...
    # calculate LFc at the replicate level
    ...
    # add quality control metrics at the sample level
    ...
    # add quality control metrics at the replicate level
...
def filter_samples(sample_metadata_with_qc):
    """
    Run quality control filtering of CRISPR screen samples. Additionally Requires at least two conditions
    with sufficient sample quality per cell line to retain the sample.

    Args:
        sample_metadata_with_qc (pandas.DataFrame): A table of sample metadata, output by `run_minipool_qc()`

    Returns:
        list[str]: A list of sample IDs passing quality control
    """
    ...
    # keep only cell lines with multiple conditions passing quality control
...
def run_chronos(readcounts, sample_metadata_to_keep, guide_metadata, neg_ctrl_guides, neg_ctrl_genes, pos_ctrl_genes, seed=41):
    """
    Runs Chronos to calculate gene effects from CRISPR readcount data.

    Args:
        readcounts (pandas.DataFrame): A matrix (sample x guide) of readcounts mapped to guides
        sample_metadata_to_keep (pandas.DataFrame): A table of sequence metadata
        guide_metadata (pandas.DataFrame): A table of guide metadata
        neg_ctrl_guides (list[str]): A list of negative control guides, expected to not have fitness effects in any cell line
        neg_ctrl_genes (list[str]): A list of negative control genes, expected to not have fitness effects in any cell line
        pos_ctrl_genes (list[str]): A list of positive control genes, expected to have strong fitness effects in any cell line
        seed (int, optional): The seed for the random number generator. Defaults to 41.

    Returns:
        pandas.DataFrame: A matrix of normalized gene effects.
        pandas.DataFrame: A table of screen metadata.
        pandas.DataFrame: A table of replicate metadata.
    """
    # prepare inputs
...
def run_linear_regression_analysis(chronos_gene_effects, full_screen_metadata, test_genes):
    """
    Perform linear regression analysis on gene effects in relation to growth format and serum-containing media.

    Args:
        chronos_gene_effects (pandas.DataFrame): A matrix (sample x gene) of gene effect scores
        full_screen_metadata (pandas.DataFrame): A table of screen metadata with covariates
        test_genes (list[str]): A list of genes to analyze

    Returns:
        pandas.DataFrame: A table of coefficients per gene representing the best-fit linear model
        pandas.DataFrame: A table of growth format coefficients and significance values
        pandas.DataFrame: A table of serum coefficients and significance values
    """
    ...
    # extract the growth format coefficients
    ...
    # extract the serum coefficients
...
def compute_geneset_variance_tests(growth_format_tests, serum_status_tests, test_gene_annotations):
    """
    Run F-tests for equality of variances among covariate coefficients for all provided gene sets

    Args:
        growth_format_tests (pandas.DataFrame): Growth format coefficents from linear regression, output by `run_linear_regression_analysis()`
        serum_status_tests (pandas.DataFrame): Serum coefficents from linear regression, output by `run_linear_regression_analysis()`
        test_gene_annotations (pandas.DataFrame): A boolean matrix (gene x geneset) of geneset membership

    Returns:
        pandas.DataFrame: A table of F-test results for each geneset
    """
    geneset_variance_df = {
        'Geneset':[], 
        'Size':[],
...
def save_minipool_data(full_screen_metadata, mp_screen_metadata, chronos_gene_effect, linear_regression_results, growth_format_differences, serum_differences, geneset_variance_df, max_gene_effect_size=-0.5):
    """
    Saves all minipool analyses and results
    """
    # gene set metadata
    ...
    # add summary statistics to growth format table
    ...
    # add summary statistics to serum table
...
def process_minipool_screens():
    """
    Perform all processing and analysis for minipool screens
    """
...
def configure_frequency_table(genes, lineages, alteration_types, tcga_sources, feature_labels):
    """
    Creates a table configuring the genes per lineage for which to summarize genomic alterations in TCGA. Ingests lists
    for all fields.
    """
    df_dict = {
        'Feature': [],
...
def compute_tcga_frequencies(alteration_metadata_df, 
    ...
    """
    Calculate the frequency of genomic alterations for selected mutations, and inserts additional entries
    containing values reported from literature.
    """
...
def compute_depmap_frequencies(alteration_metadata_df, model_subset, model_metadata, hotspot, damaging, arm_cn=None, 
    ...
    """
    Calculate the frequence of gene alterations from DepMap omics data
    """
...
def compute_selected_alteration_frequencies():
    """
    Create a table of alteration frequencies in TCGA data, reported values from literature, and occurence in DepMap
    """
...
def preprocess_addictions(fdr_threshold=0.005, correlation_threshold=0.3):
    """
    Prepare expression addiction table for plotting.

    Args:
        fdr_threshold (float, optional): Minimum FDR-corrected significance value. Defaults to 0.005.
        correlation_threshold (float, optional): Minimum correlation value. Defaults to 0.3.

    Returns:
        pandas.DataFrame: A reformatted table of expression addiction statistics
    """
    ...
    # Rename columns and add summary values
...
def preprocess_paralog_associations(variance_threshold=1, fdr_threshold=0.005, correlation_threshold=0.3):
    """
    Prepare paralog dependency table for plotting.

    Args:
        variance_threshold (int, optional): Minimum variance in biomarker. Defaults to 1.
        fdr_threshold (float, optional): Minimum FDR-corrected significance value. Defaults to 0.005.
        correlation_threshold (float, optional): Minimum correlation value. Defaults to 0.3.

    Returns:
        pandas.DataFrame: A reformatted table of paralog dependency statistics
    """
    tda_summary = load_file('tda_dependency_classification.csv', local_dir=PROCESSED_DIR, index_col=0)
...
def summarize_lineage_test_range(lineage_test_df, groupby_column='Dependency', stat_column='Mean Difference', additional_columns=['OncotreeLineage', 'Ingroup n'], require_min_ingroup_n=0):
    """
    Identify maximum and minimum lineage relationships for each oncogene or tumor suppressor -related dependencies.

    Args:
        lineage_test_df (pandas.DataFrame): A table of two-class comparison results based on lineage
        groupby_column (str, optional): Column name to group metrics by. Defaults to 'Dependency'.
        stat_column (str, optional): Column name for the test statistic. Defaults to 'Mean Difference'.
        additional_columns (list, optional): Additional columns to retain for min/max values. Defaults to ['OncotreeLineage', 'Ingroup n'].
        require_min_ingroup_n (int, optional): The minimum number of samples required to consider a group. Defaults to 0.

    Returns:
        pandas.DataFrame: A table of statistic ranges
    """
    # retrieve stats from maximum entries
    ...
    # retrieve stats from minimum entries
...
def preprocess_onc_tsg_associations(fdr_threshold=0.05, mean_difference_threshold=0.3, min_lineage_size=5):
    """
    Prepare oncogene/tumor-suppressor dependency table for plotting. Reclassifies putative relationships relative
    to the strongest lineage relationships, i.e. labels a dependency-biomarker pair as driven by a genomic
    alteration if its effect size is OUTSIDE of the range of lineage effect sizes

    Args:
        fdr_threshold (float, optional): Minimum FDR-corrected significance. Defaults to 0.05.
        mean_difference_threshold (float, optional): Minimum difference in mean dependencies. Defaults to 0.3.
        min_lineage_size (int, optional): Minimum. Defaults to 5.
    """
    ...
    # reclassify dependencies based on relative alteration / lineage effects
...
def preprocess_pdac_dependency_associations():
    """
    Prepare PDAC-Classical related dependency table for plotting.
    """
    tda_summary = load_file('tda_dependency_classification.csv', local_dir=PROCESSED_DIR, index_col=0)
    geneset_table = load_file('geneset_table_updated.csv')
    ...
    # perform geneset enrichment analysis to obtain running enrichment score
...
def prepare_wnt_example_df(wnt_genes_to_regress):
    """
    Prepare tables for plotting PDAC-classical dependencies for specific genes as examples.

    Args:
        wnt_genes_to_regress (list[str]): A list of gene names for which to extract gene effects
    """
    screen_gene_effect = load_file('screen_gene_effect.csv', index_col=0)
    screen_metadata = load_file('screen_metadata.csv', index_col=0)
...
def filter_geneset_results(ranked_diff_hypergeometric_results, min_size=50, max_size=250, fdr_threshold=0.05, top_n=None):
    """
    Filter top genesets to present for enrichment in Organoid models or Adherent models
    """
...
def cluster_genesets(significant_ranked_hypergeometric_results_df, combined_geneset_dict, min_similarity=0.3):
    """
    Cluster genesets based on the overlap between member genes

    Args:
        significant_ranked_hypergeometric_results_df (pandas.DataFrame): A table of gene set enrichment results
        combined_geneset_dict (dict[str -> list[str]]): A dictionary mapping gene sets to member genes
        min_similarity (float, optional): Minimum fraction of overlap between genes for clustering. Defaults to 0.3.

    Returns:
        pandas.Series: Cluster labels per geneset
    """
    ...
    # calculate similarity within and across clusters
...
def preprocess_global_format_comparison():
    """
    Prepare differential dependency table across organoids and adherent models.
    """
