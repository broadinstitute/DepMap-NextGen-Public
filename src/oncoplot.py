...
def load_data():
    """
    Load all data necessary for making an oncoprint
    """
...
def create_cmap_from_series(series, palette='pastel', as_hex=True):
    """
    Create a colormap based on the categories of an input series

    Args:
        series (pandas.Series): A series of labels
        palette (str, optional): The name of a colormap to use. Defaults to 'pastel'.
        as_hex (bool, optional): A flag for whether to return the colormap in hex codes. Defaults to True.

    Returns:
        dict[str -> str]: A mapping for labels to colors
    """
...
def discretize_matrix_by_interval(value_matrix, interval_dict):
    """
    Converts a continuous-valued matrix to a discrete matrix based on the specified intervals

    Args:
        value_matrix (pandas.DataFrame): A matrix of floats
        interval_dict (dict[str -> pandas.Interval]): A mapping of labels to intervals

    Returns:
        pandas.DataFrame: A matrix of categorical labels
    """
    ...
    for lb, interval in interval_dict.items():
        # assign all values in the interval to the category name
...
def generate_indicator_sequence_map(value_encoding):
    """
    Given a mapping of possible non-exclusive annotations, encodes all combinations into a unique integer using sums of powers of 2,
    and generates a corresponding binary matrix defining the label set corresponding to each combination

    Args:
        value_encoding (dict): A mapping of labels to values

    Returns:
        pandas.DataFrame: A matrix of combination encodings and their correponding component labels as columns
    """
    df_length = int(np.power(2, len(value_encoding))) # there are 2^n combinations of labels
    ...
    # generate the boolean map of components to combinations
...
def oncoprint(
    ...
    """
    Create an oncoprint (heatmap of genetic alterations) across samples and genes

    Args:
        color_matrix (pandas.DataFrame): A matrix of categorical values as entries
        marker_matrices (dict[str -> pandas.DataFrame], optional): A mapping of categories to boolean matrices. Defaults to {}.
        sample_subset (list[str], optional): A list of samples to show. Defaults to None.
        feature_subset (list[str], optional): A list of features to show. Defaults to None.
        row_sort (str, optional): The property to sort genes over. Defaults to 'frequency'.
        col_sort (list, optional): The property to sort samples over. Defaults to ['OncotreeLineage'].

    Returns:
        matplotlib.pyplot.Axes: The figure Axis for the oncoprint 
        pandas.DataFrame: A matrix with the same indices and columns as the oncoprint
    """
    ...
    if alteration_sort is not None:
        for i, alts in enumerate(alteration_sort):
            # match entries to each combination in order, if order specified
...
def categorize_mutations(
    ...
    """
    Compare mutation frequencies relative to an outgroup

    Args:
        mutation_matrix (pandas.DataFrame): A matrix (sample x gene) indicating the presence of mutations
        group_subset (list[str]): The set of samples defining the ingroup
        outgroup_subset (list[str]): The set of samples defining the ingroup
        outgroup_max_models (int, optional): The maximum number of samples in the outgroup to consider mutation presence in the ingroup as significant. Defaults to 0.
        outgroup_max_fraction (float, optional): The maximum fractions of samples in the outgroup to consider mutation presence in the ingroup as significant. Defaults to 0.2.
        matched_annotations (pandas.DataFrame, optional): Annotations to group over. Defaults to None.

    Returns:
        pandas.DataFrame: A matrix of novel mutations in the ingroup
        pandas.Series: A list of totals per label in the ingroup
        pandas.Series: A list of totals per label in the outgroup
    """
...
def expand_annotation_to_models(collapsed_matrix, model_subset, model_metadata, expansion_column='OncotreeLineage'):
    """
    Expand a group-collapsed matrix by filling values into models belonging to that group

    Args:
        collapsed_matrix (pandas.DataFrame): A matrix whose columns are groups
        model_subset (str[list]): A list of models to use
        model_metadata (pandas.DataFrame): A table of model metadata
        expansion_column (str, optional): The column in the metadata that maps models to groups. Defaults to 'OncotreeLineage'.

    Returns:
        pandas.DataFrame: An expanded matrix
    """
...
def summarize_squares(alteration_matrix, shell, ingroup, outgroup, outgroup_max_n, model_metadata):
    """
    Create a table of annotations per position in the oncoprint

    Args:
        alteration_matrix (pandas.DataFrame): A matrix of alterations
        shell (pandas.DataFrame): A matrix whose indices and columns define the sorted ordering
        ingroup (list[str]): A list of ingroup models
        outgroup (list[str]): A list of outgroup models
        outgroup_max_n (int): The maximum number of outgroup mutations to consider the presence of an ingroup mutation as significant
        model_metadata (pandas.DataFrame): A table of model metadata

    Returns:
        pandas.DataFrame: A table of index, column locations and their corresponding annotations
    """
...
def summarize_alteration_in_oncoprint(alteration_matrix, shell, ingroup, outgroup, rare_outgroup_max_n, model_metadata, alteration_type):
    """
    For a given type of alteration, generate a table summarizing the oncoprint annotations for novel and rare mutations
    ...
    Args:
        ...
        ingroup (list[str]): A list of ingroup models
        outgroup (list[str]): A list of outgroup models
        rare_outgroup_max_n (int): The maximum number of outgroup mutations to consider the presence of an ingroup mutation as significant
        model_metadata (pandas.DataFrame): A table of model metadata
        alteration_type (str): A label for the type of alteration
...
def partition_models(omics_models_meta):
    """
    Divide a model metadata table based on existence of screening data and growth format.
...
def filter_genes(hotspot, next_gen_screened_models):
    """
    Filter to a set of genes to display in the oncoprint, defined as any gene with a hotspot mutation in the set of screened NextGen models

    Args:
        hotspot (pandas.DataFrame): A matrix (sample x gene) indicating the presence of a hotspot mutation
        next_gen_screened_models (list[str]): A list of models to search for hotspot mutations in
...
def create_large_oncoprint(cn_transform, hotspot, damaging, mutation_matrix, omics_models_meta, ingroup_models, outgroup_models, genes_to_oncoprint, lineage_order, figsize):
    """
    Generate a large oncoprint for all models

    Returns:
        pandas.DataFrame: A table summarizing all annotations to highlight
    """
...
def get_alteration_shell(alteration_matrix, ingroup, outgroup, model_metadata, gene_subset, model_n=0):
    """
    Calculate the indices and columns that are subset to relevant annotations to highlight, namely those mutations which
    are novel in the NextGen dataset
    ...
    Args:
        ...
        outgroup (list[str]): A list of outgroup models
        model_metadata (pandas.DataFrame): A table of model metadata
        gene_subset (list[str]): A list of genes to show
        model_n (int, optional): The maximum number of mutations in outgroup models to keep a gene for display. Defaults to 0.
    """
...
def create_condensed_oncoprint(cn_transform, hotspot, damaging, mutation_matrix, omics_models_meta, genes_to_oncoprint, lineage_order):
    """
    Generate an oncoprint showing all mutations that are novel in lineage for the NextGen dataset
    """
