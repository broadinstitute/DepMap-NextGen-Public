...
if (file.exists(file.path(INPUT_PATH, "mONCTSG_Gene_Result.txt"))) {file.remove(file.path(INPUT_PATH, "mONCTSG_Gene_Result.txt"))}
if (file.exists(file.path(INPUT_PATH, "mONCTSG_MutFree_Result.txt"))) {file.remove(file.path(INPUT_PATH, "mONCTSG_MutFree_Result.txt"))}
if (file.exists(file.path(INPUT_PATH, "mONCTSG_Pathway_Result.txt"))) {file.remove(file.path(INPUT_PATH, "mONCTSG_Pathway_Result.txt"))}
...
for(i in 5:ncol(mONCTSG_Pathway)) {
  ...
  tteste<-ttest$estimate
  ttesp<-ttest$p.value
  cat(i,tteste,ttesp,mwup,"\n",file=file.path(INPUT_PATH, "mONCTSG_Pathway_Result.txt"), append=TRUE)
}
